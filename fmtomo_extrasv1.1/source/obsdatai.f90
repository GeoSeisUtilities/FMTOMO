...
PROGRAM obsdata
IMPLICIT NONE
INTEGER :: i,j,jj,k,kk,nisf,nps,ns,nsif,nrif,extt
INTEGER :: lot,npr,nr,istep,checkstat,mxnps
INTEGER :: nvg,csdif,nsi,extsr,crid,srcid
...
REAL, DIMENSION (:), ALLOCATABLE :: rlats, rlons, rdeps
REAL, PARAMETER :: lltol=1.0e-5, dptol=1.0e-5
CHARACTER (LEN=10) :: phase
CHARACTER (LEN=30) :: ifiles,ifiler,ofiles,ofiler,ofilesd
CHARACTER (LEN=30) :: ifilevg,ifilerd,ofilet,cdum,srcf
!
...
! extsr = Extract source-receiver information
! srcf = Source-receiver info. file
! crid = current receiver ID
! rlats, rlons, rdeps = Saved receiver coordinates
! lltol,dptol = tolerances on location difference
! srcid = source id
!
OPEN(UNIT=10, FILE='obsdatai.in',STATUS='old')
READ(10,*)
READ(10,*)
READ(10,*)
READ(10,*)
...
! the traveltime file. Also, create the source-receiver association file
! if necessary.
!
...
ENDIF
IF(extsr.EQ.1)THEN
   OPEN(UNIT=90,FILE=srcf,STATUS='unknown')
   WRITE(90,*)ns
   crid=0
   ALLOCATE(rlats(nr+1),rlons(nr+1),rdeps(nr+1))
ENDIF
istep=0
siter=0
riter=0
      ...
      DO jj=1,npath
         ...
         DO k=1,nrif
            ...
            IF(extsr.EQ.1)THEN
               ...
               IF(crid.GT.1)THEN
                  DO kk=crid-1,1,-1
                     IF(ABS(rlats(crid)-rlats(kk)).LT.lltol)THEN
                        IF(ABS(rlons(crid)-rlons(kk)).LT.lltol)THEN
                           IF(ABS(rdeps(crid)-rdeps(kk)).LT.dptol)isw=1
                        ENDIF
                     ENDIF
                     IF(isw.EQ.1)THEN
                        crid=crid-1
                        WRITE(90,*)kk,tt,tterr
                        EXIT
                     ENDIF
                  ENDDO
                  IF(isw.EQ.0)WRITE(90,*)crid,tt,tterr
               ELSE
                  WRITE(90,*)crid,tt,tterr   
               ENDIF
            ENDIF
