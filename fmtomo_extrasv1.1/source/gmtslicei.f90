...
! parv = plot absolute (0) or relative (1) velocity or % (2)
...
!
sep='>'
OPEN(UNIT=10,FILE='gmtslicei.in',STATUS='old')
!
         ...
         DO i=0,nnr(ii)+1
            DO j=0,nnt(ii)+1
               DO k=0,nnp(ii)+1
                  READ(20,*)rd1
                  IF(nvgt.EQ.1.OR.nvgt.EQ.2.AND.ips.EQ.2)THEN
                     rd2=veln(nnr(ii)+1-i,nnt(ii)+1-j,k,ii)-rd1
                     IF(parv.EQ.2)rd2=100.*rd2/rd1
                     veln(nnr(ii)+1-i,nnt(ii)+1-j,k,ii)=rd2
                  ENDIF
               ENDDO
            ENDDO
         ENDDO
