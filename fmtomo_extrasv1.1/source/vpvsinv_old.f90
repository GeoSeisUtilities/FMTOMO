...
USE globalp
IMPLICIT NONE
INTEGER :: i,j,k,maxf,istep,jstep,jup,nrow,cstp,idum
INTEGER :: tstat,stnm,iscn,isum,ihd1,ihd2
INTEGER, DIMENSION (:), ALLOCATABLE :: stpv,ntsrc
INTEGER, PARAMETER :: maxss=50
REAL(KIND=i5) :: fracg
REAL(KIND=i5), DIMENSION (:), ALLOCATABLE :: srcm
REAL(KIND=i5), DIMENSION (:,:,:), ALLOCATABLE :: vpvs
CHARACTER (LEN=25) :: gridi,gridc,otimes,mtimes
CHARACTER (LEN=25) :: frechet,sarf
!
...
! iscn = Source number
! sarf = File containing numbers of data
! ihd1,ihd2 = Header of grid file
!
! Read in the input parameters
!
OPEN(UNIT=10,FILE="vpvsinv.in",STATUS='old')
READ(10,*)
READ(10,*)
READ(10,*)
READ(10,1)gridi
...
! receiver array.
!
OPEN(UNIT=10,FILE=sarf,STATUS='old')
READ(10,*)ntr
CLOSE(10)
!
! Now read in reference grid and current grid
!
...
!
ALLOCATE(vpvs(0:nvr+1,0:nvt+1,0:nvp+1), STAT=checkstat)
IF(checkstat > 0)THEN
...
DO i=1,nvr+2
   DO j=1,nvt+2
      DO k=1,nvp+2
         idum=(k-1)*(nvt+2)*(nvr+2)+(nvt+2-j)*(nvr+2)+nvr+2-i+1
         READ(10,*)mo(idum)
         cm(idum)=1.0
      ENDDO
   ENDDO
...
DO i=0,nvr+1
   DO j=0,nvt+1
      DO k=0,nvp+1
         READ(10,*)vpvs(nvr+1-i,nvt+1-j,k)
      ENDDO
   ENDDO
ENDDO
CLOSE(10)
...
cnfe(0)=0
DO i=1,ntr
   READ(10,*)tstat,dobs(istep)
   cd(istep)=1.0
   READ(20,*)idum,dmod(istep)
   IF(tstat.NE.idum)THEN
      WRITE(6,*)'Warning, observed and model data files'
      WRITE(6,*)'are inconsistent!!!!'
      WRITE(6,*)'Check traveltime number ',i
   ENDIF
   READ(40)nrow
   cd(istep)=cd(istep)**2
...
DO i=0,nvr+1
   DO j=0,nvt+1
      DO k=0,nvp+1
         WRITE(10,*)vpvs(nvr+1-i,nvt+1-j,k)
      ENDDO
   ENDDO
ENDDO
CLOSE(10)
