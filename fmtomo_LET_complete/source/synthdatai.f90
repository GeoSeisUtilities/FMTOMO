...
INTEGER telegn,nrc,nrct,nsct
INTEGER, PARAMETER :: maxiter=100000000, maxr=1000, maxs=5000
INTEGER, DIMENSION(:), ALLOCATABLE :: tsid
INTEGER, DIMENSION(maxr) :: rcn,nsrc
INTEGER, DIMENSION(maxs,maxr) :: srn
...
CHARACTER(LEN=30) :: ofile,ifile,scfile,rtfile,srcf,srcfn
!
...
! srcf = Source-receiver association file
! srcfn = New srcf file
! maxr,maxs = maximum receivers, maximum sources
! rcn = Receiver number
! nsrc = Number of sources detected by receiver rcn
! srn = Source id for receiver rcn
! nrc = Number of receivers
! nrct = Counter over number of receivers
! nrst = Counter over number of sources
!
OPEN(UNIT=10,FILE='synthdatai.in',STATUS='old')
READ(10,'(a26)')ifile
READ(10,*)cv
READ(10,'(a26)')ofile
READ(10,'(a26)')scfile
...
ENDIF
OPEN(UNIT=50,FILE=srcf,STATUS='old')
READ(50,*)nrc
DO i=1,nrc
   READ(50,*)rcn(i),nsrc(i)
   DO j=1,nsrc(i)
      READ(50,*)srn(j,i)
   ENDDO
ENDDO
CLOSE(50)
OPEN(UNIT=60,FILE=srcfn,STATUS='unknown')
WRITE(60,*)nrc
WRITE(20,*)nr
nrct=1
nsct=0
WRITE(60,*)rcn(nrct),nsrc(nrct)
DO i=1,nr
   nsct=nsct+1
   IF(nsct.GT.nsrc(nrct))THEN
      nrct=nrct+1
      nsct=1
      WRITE(60,*)rcn(nrct),nsrc(nrct)
   ENDIF
