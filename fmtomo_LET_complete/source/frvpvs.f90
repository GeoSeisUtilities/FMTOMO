...
REAL :: dvr,dvx,dvz,tobs,plt,tobse
...
REAL, DIMENSION(maxr,maxs) :: tto,ttoe
...
! ttoe = Observed traveltime errors
...
DO i=1,nrp
   ...
   DO j=1,nslr(i)
      READ(40,*)idtto(i,j)
      READ(60,*)id1,id2,id3,id4,tto(i,j),ttoe(i,j)
   ENDDO
ENDDO
spas=-1
nmp=0
DO i=nrp+1,nr
   ...
   DO j=1,nslr(i)
      READ(40,*)idtto(i,j)
      READ(50,*)id1,id2,id3,id4,tto(i,j),ttoe(i,j)
...
DO i=nrp+1,nr
   DO j=1,nslr(i)
      IF(spas(i,j).GT.0)THEN
         ...
         tobse=ttoe(i,j)**2+(vpvsr*ttoe(idr(i),spas(i,j)))**2
         tobse=sqrt(tobse)
         rd1=rd1+tobs**2
      ELSE
         id1=0
