...
!
! Set some values to account for ttimes not returning specific
! requests for PKP branches.
!
      p1='PKPab'
      p2='PKPbc'
      p3='PKPdf'
      p4='PKP'
      if(tele_phase.eq.p1.or.tele_phase.eq.p2.or.tele_phase.eq.p3)then
         phlst(1)=p4
      else
         phlst(1)=tele_phase
      endif
...
itloop: do while (abs(f3) > 1.e-7*gt_norm  .and. abs(deltan2-deltan1) > 1.e-6*deltas) 
      ...
      ! if solution deteriorates rather than improves, switch to bisection
        ...
        if (phid == 0)then
            write(6,*)'phase does not exist s3'
            reg%init_arrivaltime(m)=huge_time
            reg%init_time_gradient(1:3,m) = 0.d0
            cycle nodeloop
        endif
...
!
     ...
     stt3=stt3+etcor
      ...
      if(loc_phase.eq.p1.or.loc_phase.eq.p2.or.loc_phase.eq.p3)then
         phlst(1)=p4
      else
         phlst(1)=loc_phase
      endif
...
! (raypar) and tg_radial ( = dtdh, the arrival time derivative wrt source depth) as output
      ...
      character(len=8) :: phase,n_phase,b_phase,g_phase,p1,p2,p3,p4
      integer  :: mm
...
!
      ...
      if(phase.eq.p1.or.phase.eq.p2.or.phase.eq.p3)then
         phlst(1)=p4
      else
         phlst(1)=phase
      endif
...
!
!   Correct for PKP phases if required.
!
        if(phlst(1).eq.p4)then
           k=1
           phid=0
           do while(k.le.mm.and.phid.eq.0)
              if(phcd(k).eq.phase)then
                 phid=k
              else
                 k=k+1
              endif
...
! decompose into parallel and perpendicular components
 ...
 ! calculate perpendicular component on the inside of the boundary
  ...
  if (inside_hits>0) then
     ...
     print *,'Warning - inside node hits from teleseismic wave'
  endif
  if (n_tot_ref > 0) then
