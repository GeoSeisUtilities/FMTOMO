...
real(kind=dp) :: lat,long,ltol
real(kind=dp), parameter :: tolr=0.01
type(Tinterface)  :: iface

integer        :: i,j,ilat,ilong,isw
...
if (ilong < 2 .or. ilong > (iface%nlong-2)) then 
   ...
   ltol=tolr*iface%dlong0
   if(ilong > (iface%nlong-2))then
      if(floor((long-iface%long0-ltol)/iface%dlong0)+1.eq.(iface%nlong-2))then 
         print *,'making correction'
         ilong=ilong-1
         isw=1
      endif
   else if(ilong < 2)then
      if(floor((long-iface%long0+ltol)/iface%dlong0)+1.eq.2)then
         print *,'making correction'
         ilong=ilong+1
         isw=1
      endif
   ...
   if(isw.eq.0)then
      print *,'Error with interpolate_interface'
      print *,iface%id,ilong,ilat
      print *,iface%nlong, floor((long-iface%long0-tolr)/iface%dlong0)
      print *,long,lat
...
if (ilat < 2 .or. ilat > (iface%nlat-2)) then
   ...
   ltol=tolr*iface%dlat0
   if(ilat > (iface%nlat-2))then
      if(floor((lat-iface%lat0-ltol)/iface%dlat0)+1.eq.(iface%nlat-2))then 
         print *,'making correction'
         ilat=ilat-1
         isw=1
      endif
   else if(ilat < 2)then
      if(floor((lat-iface%lat0+ltol)/iface%dlat0)+1.eq.2)then
         print *,'making correction'
         ilat=ilat+1
         isw=1
      endif
